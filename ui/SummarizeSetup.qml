...
// Shown when a summarize key is pressed but no provider is configured. Adaptive:
// detects which agent CLIs are logged in on THIS machine and offers a one-click
// keyless button per one (no key, runs on the existing plan); and always offers
// an API-key path where you pick the vendor (Anthropic or OpenAI) and paste a
// key. Both write the summarize block to accounts.json via the daemon.
Modal {
    id: gd
    ...
    Column {
        ...
        Column {
            ...
            Text {
                width: parent.width
                text: "A small, fast model is used (" + (gd.vendor === "anthropic" ? "Haiku" : "gpt-4o-mini") + "). The key is stored in accounts.json; change the model there later."
                color: Theme.fg_muted; wrapMode: Text.Wrap
                font.family: Theme.fontFamily; font.pixelSize: 12; lineHeight: 1.4
            }
        }
        ...
        Text {
            width: parent.width
            text: "Other providers (local Ollama, etc.): add a summarize block to ~/.config/mlqs/accounts.json."
            color: Theme.fg_muted; wrapMode: Text.Wrap
            font.family: Theme.fontFamily; font.pixelSize: 12; lineHeight: 1.4
        }
    }
